...
const ApproachAnimation = () => {
  ...
  return (
    ...
    >
      <div
        ref={sectionRef}
        className="sticky top-0 md:h-[100vh] flex flex-col md:flex-row bg-white transition-opacity duration-500 "
      >
        {/* Left Panel */}
        <div className="w-full md:w-1/2 flex justify-center items-start md:items-center px-4 md:px-6 mt-6 md:mt-0 pt-16  md:pt-10 h-[30vh] md:h-screen md:sticky md:top-0 bg-white ">
        ...
        {/* Right Panel */}
        <div className="w-full md:w-1/2 h-[60vh] md:h-screen overflow-hidden relative my-14 md:my-0 ">
          <div className="h-full w-full relative">
            {blocks.map((block, index) => (
              <div
                key={index}
                ref={(el) => (blocksRef.current[index] = el)}
                className={`absolute top-0 left-0 w-full h-full flex justify-center items-center px-4 sm:px-6 md:px-12 transition-all duration-700 ease-out
                  ${
                    index === currentSlide
              ...
              >
                {block.type === "text" ? (
                  <div className="max-w-xl px-2 sm:px-4 md:px-0">
                    <h3
                      className="text-2xl md:text-3xl font-ebold text-[#2050B1] mb-2 md:mb-4 uppercase font-tthoves-bold"
                      dangerouslySetInnerHTML={{ __html: block.title }}
                    ></h3>
                    <p className="text-sm md:text-xl font-tthoves-extralight">
                      {block.text}
                    </p>
                  </div>
                ) : (
                  <img
                    src={block.src}
                    alt={block.alt}
                    className="w-full h-auto max-h-[50vh] sm:max-h-[60vh] md:max-h-[80vh] object-contain px-2 sm:px-4 md:px-0"
                  />
                )}
              </div>
            ))}
